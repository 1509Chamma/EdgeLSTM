...
class TestONNXParser(unittest.TestCase):
    ...
    def test_matmul_add_parse(self):
        ...
        node1 = helper.make_node("MatMul", ["X", "W"], ["MatMul_out"], name="matmul_node")
    ...
    def test_lstm_parse(self):
        ...
        graph = helper.make_graph(
            ...
            "lstm_graph",
            [
                helper.make_tensor_value_info("X", TensorProto.FLOAT, [5, 1, input_size]),
                helper.make_tensor_value_info("W", TensorProto.FLOAT, [1, 4 * hidden_size, input_size]),
                helper.make_tensor_value_info("R", TensorProto.FLOAT, [1, 4 * hidden_size, hidden_size]),
            ],
            [helper.make_tensor_value_info("Y", TensorProto.FLOAT, [5, 1, 1, hidden_size])],
        )
