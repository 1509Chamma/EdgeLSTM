...
from edgelstm.ops.builtins import (
    MatMul, Add, Mul, Div, Transpose, Reshape, Concat, Slice,
    Sigmoid, Tanh, ReLU, GELU, Softmax, Sum, Mean, Max,
    LayerNorm, Conv1D, Pad, Shift, LSTM, InvalidOperatorInstanceError
)
...
def test_matmul_validation_edge_cases():
    ...
    with pytest.raises(InvalidOperatorInstanceError, match="supports rank-2 tensor inputs only"):
        op.validate(values)
    ...
    with pytest.raises(InvalidOperatorInstanceError, match="requires lhs.shape\[1\] == rhs.shape\[0\]"):
        op.validate(values)
...
def test_concat_validation_edge_cases():
    ...
    with pytest.raises(InvalidOperatorInstanceError, match="non-concatenated dimensions to match"):
        op.validate(values)

def test_slice_validation_edge_cases():
    op = Slice(op_id="s1", inputs=["a"], outputs=["b"], attrs={"axis": 0, "start": 0, "end": 5})
    # End > dimension
    values = {
        "a": make_tensor("a", [3]),
        "b": make_tensor("b", [3])
    }
    with pytest.raises(InvalidOperatorInstanceError, match="start < end <= input dimension"):
        op.validate(values)
    
    # Negative step
    op = Slice(op_id="s1", inputs=["a"], outputs=["b"], attrs={"axis": 0, "start": 0, "end": 2, "step": -1})
...
def test_conv1d_validation_edge_cases():
    ...
    with pytest.raises(InvalidOperatorInstanceError, match="input channels to match weight channels"):
        op.validate(values)

def test_lstm_validation_edge_cases():
    op = LSTM(op_id="l1", inputs=["x", "w", "r"], outputs=["y"], attrs={"hidden_size": 128})
    # Input X rank not 3
    values = {
    ...
    values = {
        ...
        "w": make_tensor("w", [1, 512, 64]), # should be 4*hidden_size = 512, but hidden_size=128, so it matches. Wait.
        "r": make_tensor("r", [1, 512, 128]),
        "y": make_tensor("y", [10, 1, 1, 128])
    }
...
def test_pad_validation_edge_cases():
    ...
    values = {
        "in": make_tensor("in", [10, 10]),
        "out": make_tensor("out", [12, 12])
    }
...
def test_reduction_axes_edge_cases():
    ...
    # Invalid axis type
    op.attrs["axis"] = "zero"
    with pytest.raises(InvalidOperatorInstanceError, match="integer or sequence of integers"):
        op.validate(values)
...
def test_binary_mismatch():
    ...
    with pytest.raises(InvalidOperatorInstanceError, match="matching tensor shapes or scalar broadcasting"):
        op.validate(values)
...
def test_built_in_cost_estimation():
    ...
    values = {"a": make_tensor("a", [2, 2]), "b": make_tensor("b", [2, 2]), "c": make_tensor("c", [2, 2])}
    ...
    # Transpose cost
    op_trans = Transpose(op_id="t1", inputs=["a"], outputs=["b"], attrs={"perm": [1, 0]})
