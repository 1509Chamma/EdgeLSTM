...
if TYPE_CHECKING:
    ...
    from edgelstm.ir.op import Operator
...
def validate_graph(graph: Graph) -> None:
    ...
    for input_id in graph.graph_inputs:
        if input_id not in graph.values:
            raise GraphValidationError(f"Graph input '{input_id}' not found in values", input_id)

    for output_id in graph.graph_outputs:
        if output_id not in graph.values:
            raise GraphValidationError(f"Graph output '{output_id}' not found in values", output_id)
        ...
        for input_id in op.inputs:
            if input_id not in graph.values:
                raise GraphValidationError(
                    f"Operator '{op_id}' references non-existent input '{input_id}'", op_id
                )
        for output_id in op.outputs:
            if output_id not in graph.values:
                raise GraphValidationError(
                    f"Operator '{op_id}' references non-existent output '{output_id}'", op_id
                )
    ...
    for val_id, val in graph.values.items():
        if val.producer_op_id:
            if val.producer_op_id not in graph.ops:
                raise GraphValidationError(
                    f"Value '{val_id}' references non-existent producer_op_id '{val.producer_op_id}'", val_id
                )
            ...
            if val_id not in producer.outputs:
                raise GraphValidationError(
                    f"Value '{val_id}' claims to be produced by '{val.producer_op_id}', but is not in its outputs", val_id
                )
    ...
    for val_id, val in graph.values.items():
        ...
        for dim in val.shape:
            if not isinstance(dim, int) or dim <= 0:
                raise ValueValidationError(f"Value '{val_id}' shape must contain positive integers", val_id)
        ...
        if len(val.axes) != len(val.shape):
            raise ValueValidationError(
                f"Value '{val_id}' axes length ({len(val.axes)}) must match shape length ({len(val.shape)})", val_id
            )
            
        # Dtype and VType validation
        if not isinstance(val.dtype, str) or not val.dtype:
            raise ValueValidationError(f"Value '{val_id}' must have a valid string dtype", val_id)
            
        if val.vtype not in allowed_vtypes:
            raise ValueValidationError(f"Value '{val_id}' has invalid vtype: {val.vtype}", val_id)
            
        # Quantization consistency (basic check)
        if val.quant:
            if not isinstance(val.quant, dict):
                raise ValueValidationError(f"Value '{val_id}' quantization config must be a dictionary", val_id)


def validate_operators(graph: Graph) -> None:
    """
    Ensure all operators are valid according to their own internal rules and general IR rules.
    """
    for op_id, op in graph.ops.items():
        # Internal validation
        try:
            ...
            raise OperatorValidationError(f"Internal validation failed for operator '{op_id}': {str(e)}", op_id) from e
        ...
        if not op.inputs and op.op_type != "Constant": # constants might have no inputs
             ...
             # Requirement says: "no empty input/output lists (unless explicitly allowed)"
             pass 
...
def validate_topology(graph: Graph) -> None:
    ...
    for val_id in graph.values:
        if val_id not in reachable_values:
             raise TopologyValidationError(f"Value '{val_id}' is unreachable from inputs", val_id)

    for op_id in graph.ops:
        if op_id not in reachable_ops:
            raise TopologyValidationError(f"Operator '{op_id}' is unreachable from inputs", op_id)

    # Ensure all graph_outputs are reachable (ensure all outputs have producers or are inputs)
    for output_id in graph.graph_outputs:
        if output_id not in reachable_values:
             raise TopologyValidationError(f"Graph output '{output_id}' is unreachable from inputs", output_id)
...
def validate_fpga_constraints(graph: Graph, device: FPGADevice) -> None:
    ...
    if total_lut > device.resources.luts:
        raise IRValidationError(
            f"Insufficient LUTs on device '{device.name}': required {total_lut}, available {device.resources.luts}"
        )
    if total_dsp > device.resources.dsps:
        raise IRValidationError(
            f"Insufficient DSPs on device '{device.name}': required {total_dsp}, available {device.resources.dsps}"
        )
    # Simplified BRAM check (36k blocks)
    if total_bram > device.resources.bram_36k:
        raise IRValidationError(
            f"Insufficient BRAM on device '{device.name}': required {total_bram}, available {device.resources.bram_36k}"
        )


def validate_ir(graph: Graph, device: Optional[FPGADevice] = None) -> None:
    """
